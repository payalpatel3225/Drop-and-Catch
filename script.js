...
// Handle basket drag functionality for mobile
let isDragging = false;
let startX = 0;

function enableBasketDrag() {
    ...
    basket.addEventListener("touchmove", (e) => {
        ...
        basketLeft = Math.max(0, Math.min(100, basketLeft + (deltaX / window.innerWidth) * 100));
...
// Enable drag functionality for mobile screens
function checkScreenSize() {
    if (window.innerWidth <= 600) {
        enableBasketDrag(); // Call the drag function instead of click
    }
}
checkScreenSize();
